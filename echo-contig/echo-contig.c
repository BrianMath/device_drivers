...
// Não necessário por ser usado somente localmente
...
echo_write(struct cdev *dev, struct uio *uio, int ioflag) {
    ...
    // Adiciona um '\0' ao final da string
...
echo_modevent(module_t mod __unused, int event, void *arg __unused) {
    ...
    switch (event) {
	case MOD_LOAD:
	    echo_message = contigmalloc(sizeof(echo_t), M_ECHO, M_WAITOK | M_ZERO,
	    ...
	    break;
	case MOD_UNLOAD:
	    // destroy_dev() retira o dispositivo de /dev
